        ...
        <div class="md:w-1/2 p-5">
            ...
            <div>
                <div class="flex items-center gap-2 mr-5">
                    ...
                    <p class="gap-2">{{$post->descripcion}}</p>
                </div>
                <p class="text-sm text-gray-600">{{$post->created_at->diffForHUmans()}}</p>
            </div>

            @auth
                @if ($post->user_id === auth()->user()->id) {{-- La persona que esta autenticada es la misma que creo el post? --}}
                    ...
                    @if ($post -> comentarios->count())
                        ...
                        <div class="p-5 border-gray-300 border-b">
                            <div class="flex gap-3">
                                <img src="{{ $comentario->user->imagen ? asset('perfiles').'/'. $comentario->user->imagen: asset('img/avatar.jpg')}}" alt="Imagen" class="rounded-full w-12 h-12">  
                                <div class="flex flex-col">
                                    <a href="{{ route('posts.index', $comentario->user) }}" class="font-black italic">
